...
def inference(source_prompt, target_prompt, positive_prompt, negative_prompt, local, mutual, guidance_s, guidance_t, num_inference_steps=10,
    ...
    ptp_utils.register_attention_control(pipe, controller) # check
...
def run_inference(annotation_idx, manual_params, annotation, root, target, base_params):  
    """
    ...
    root: путь по изобрадения
    target: путь до папки сохранения изобрадения
    base_params: базовые параметры из конфига запуска
    """
    
    print(annotation_idx)
    image_path = os.path.join(root, "annotation_images", annotation["image_path"])
    ...
    save_path = os.path.join(target, "annotation_images", os.path.dirname(annotation["image_path"]))
    os.makedirs(save_path, exist_ok=True)
    unique_filename = get_unique_filename(save_path, os.path.basename(annotation["image_path"]))
    image_out.save(os.path.join(save_path, unique_filename))


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument('--config', type=str, required=True, help='Path to experiment config JSON file')
    ...
    target_origin = config["target_path"]
    target = os.path.join(config["target_path"], config["exp_name"])
    ...
    for image_id, manual_params  in config["images"]:
        if image_id in annotation_file:
            run_inference(image_id, manual_params, annotation_file[image_id], root, target, base_params)
        else:
    ...
    #     print(f"#####_____{param}_____######")
    #     for i in grid_parameters[param]:
    #         tmp_base_params = base_params.copy()
    #         if param=="strength" and i!=0:
    #             tmp_base_params["denoise"] = True
    #         tmp_base_params[param] = i
    #         target = os.path.join(target_origin, param, f"{i}_{param}")
        
    #         for image_id, manual_params  in config["images"]:
    #             if image_id in annotation_file:
    #                 run_inference(image_id, manual_params, annotation_file[image_id], root, target, tmp_base_params)
    #             else:
    #                 print(f"Annotation not found for image ID: {image_id}")
...
# python run_model.py --config experiments/configs/example.json
